...
phi= randn*pi;
f= 0.1;
y= exp(-vx.^2/(2*10^2)).*sin(2*pi*f*vx + phi) ;  % sin(2*pi*f*vx + pi/2 + phi-pi/2)
...
%%  evolope 
% y_hilbert= hilbert(y);
% y_envolope= sqrt(real(y_hilbert).^2+imag(y_hilbert).^2);
% y_envolope= smooth(y_envolope,5);
% 
% figure(3);clf
% plot(vx,y,'b-');hold on;grid on
% plot(vx,y_envolope,'r--');
